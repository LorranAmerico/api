...
def buscar_calcinha_mais_cara():
    url = f"{API_URL}?SubGrupo1=CALCINHAS"
    try:
        response = requests.get(url, timeout=5)
        response.raise_for_status()
        dados = response.json()
        ...
        calcinha_mais_cara = max(dados, key=lambda x: float(x.get("Venda_Valor", 0)))
...
def buscar_top5(tipo, ordem="mais barato"):
    url = f"{API_URL}?SubGrupo1={tipo.upper()}"
    try:
        response = requests.get(url, timeout=5)
        response.raise_for_status()
        dados = response.json()
        ...
        if ordem == "mais caro":
            produtos_ordenados = sorted(dados, key=lambda x: float(x.get("Venda_Valor", 0)), reverse=True)
        else:
            produtos_ordenados = sorted(dados, key=lambda x: float(x.get("Venda_Valor", 0)))
...
def interpretar_mensagem(user_message):
    ...
    if numeros and any(palavra in user_message.lower() for palavra in palavras_chave):
        ...
        return (f"Aqui estão os detalhes da referência {codigo}:\n"
                ...
                f"- Tipo: {resultado.get('SubGrupo1', 'N/A')}\n"
